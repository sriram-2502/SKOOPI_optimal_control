    ...
    for i = 1: n_dim
        if(real(D(i,i))<0)
            ...
            if(norm(x_local - x_op) <= 1e-3)
                ...
                phi_x_op_algebra(i)   = phi_algerba.phi;

                % threshold values close to zero
                % if(abs(phi_complete(idx, idx_lambda_max))<1e-4)
                %     divide = sign(phi_complete(idx, idx_lambda_max))*1e-4;
                % else
                %     divide = phi_complete(idx, idx_lambda_max);
                % end
                divide = phi_complete(idx, idx_lambda_max);

                phi_x_op(i)           = phi_algerba.phi/divide^k;
...
phi.phi_x_op_algebra   = phi_x_op_algebra;
